...
#'
#' @examples
download.annotation.key <- function(species,maxOrganism=20,indextostart,accessionDir,outDir)
{
  library(reutils)
  library(ape)
  library(seqinr)
  ...
  for(i in indextostart:n.species)
  {
    ...
    for(j in 1:min(c(maxOrganism,N.accession)))
    {
      ...
      for(k in 1:N.chromosomes)
      {
        ...
        {
          if(all(is.element(c('start','end','type','product'),colnames(current.annotation))))
          {
